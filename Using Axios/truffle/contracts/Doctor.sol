...
contract Doctor { 
  ...
  function addDoctor(address _doctorId, string memory _doctorName, string memory _clinicName, string memory _dept, string memory _password) 
           public {
    require(doctors[_doctorId].id != _doctorId, "This doctor already exists.");

    DoctorDetails memory doctor = DoctorDetails(_doctorId, _doctorName, _clinicName, _dept, _password);
  ...
  function doctorAddPatient(address _patientId, string memory _name, string memory _department, string memory _password) public senderIsDoctor{
    Patient patientContract = Patient(patientContractAddress);
    
    patientContract.addPatient(_patientId, _name, _department, _password);
